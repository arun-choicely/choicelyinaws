sudo apt update
sudo apt install nodejs
sudo apt install npm
sudo npm install -g npm@latest
npm -v
npx -v
node -v
ls
npx create-react-app brand-influencer-platform
cd brand-influencer-platform/
npm start
ls
cd src
vi App.js 
ls
vi HomePage.js
vi CreateProfile.js
cd ..
npm start
cd src
vi App.js 
cd ..
npm start
npm install react-router-dom
cd src
vi BrandProfile.js
vi CreatorProfile.js
ls
mkdir styles
cd styles
vi styles.css
cd brand-influencer-platform/
ls
cd src
ls
cd styles/
ls
vi styles.css 
cd ..
vi App.js 
ls
cd ..
ls
cd ..
mkdir backend
ls
cd backend
npm init -y
vi app.js
cd ..
cd brand-influencer-platform/
l;s
cd src
ls
vi BrandProfile.js 
cd ..
ls
cd backend
ls
npm install express mongoose cors
vi app.js 
mkdir models
cd models
npm install mongoose
vi UserProfile.js
touch UserProfile.js 
touch Partnership.hs
vi Partnership.hs 
rm Partnership.hs 
ls
touch Partnership.js
vi Partnership.js 
cd ..
vi app.js 
npm start
ls
vi package.json 
cls
clear
vi package.json 
cd ..
cd brand-influencer-platform/
ls
cd src
ls
vi config.js
curl http://checkip.amazonaws.com/
vi config.js
cd backend/
vi package.json 
npm start
ls
vi app.js 
npm start
vi app.js 
npm start
vi app.js 
cd models/
ls
vi UserProfile.js 
cd ..
ls
touch .env
vi .env 
nano .env
vi .env
npm install dotenv
vi app.js 
vi .gitignore
npm start
npm install cors
vi package.json 
npm start
vi app.js 
vi .env 
npm start
vi .env 
npm start
vi .env 
npm start
vi .env 
npm start
vi .env 
npm start
nslookup cluster0.mongodb.net
nslookup cluster0-shard-00-02.ls47o.mongodb.net
vi .env 
npm start
nslookup cluster0-shard-00-02.ls47o.mongodb.net
vi .env 
vi app.js 
npm start
nslookup cluster0-shard-00-02.ls47o.mongodb.net
vi app.js 
vi .env 
npm start
vi .env 
npm start
cd brand-influencer-platform/
ls
cd src
vi config.js 
cd ..
npm start
npm install --save-dev @babel/plugin-proposal-private-property-in-object
ls
vi package.json 
npm install -g npm-check-updates
ncu
sudo apt install nsight-compute
npm run build
cd src
ls
vi App.js 
mkdir pages
mv HomePage.js ./pages/
mv CreatProfile.js ./pages/
mv CreateProfile.js ./pages
mv CreatorProfile.js ./pages/
mv BrandProfile.js  ./pages/
ls
cd pages
ls
cd ..
vi App.js 
cd ..
ls
npm run build
npm start
sudo systemctl status mongod
sudo apt update
sudo apt install git
git config --global user.name "arun@choicely.ai"
git config --global user.email "arun@choicely.ai"
git init
git remote add origin https://github.com/your-username/your-repo.git
ls
git add .
git commit -m "Your commit message"
git commit -m "Initial commit"
git push -u origin master
git remote -v
git remote set-url origin https://github.com/your-username/your-repo.git
git init
git add .
git commit -m "Initial commit"
git status
cd brand-influencer-platform/
git status
git add .
git commit -m "Initial commit"
git remote -v
git remote add origin https://github.com/arun-choicely/Choicely-aws
git remote set-url origin <repository-url>
git remote set-url origin https://github.com/arun-choicely/Choicely-aws
git remote -v
git push -u origin master
git credential-cache exit
git push -u origin master
git remote add origin https://github.com/arun-choicely/Choicely-aws.git
git remote -v
git remote add origin https://github.com/arun-choicely/Choicely-aws.
git remote set-url origin https://github.com/arun-choicely/Choicely-aws.git
git remote -v
git push -u origin master
git credential-cache exit
git push -u origin master
ls
cd ..
ls
cd backend/
ls
cd ..
cd backend/
cd ..
git add backend/
git status
git commit -m "Added backend"
cd backend/
git commit -m "Initial checkin of backend"
cd ..
git init
git add backend brand-influencer-platform
git commit -m "Initial check in"
git remote add origin https://github.com/arun-choicely/Choicely-aws.git
git remote -v
git remote add origin https://github.com/arun-choicely/Choicely-aws.git
git remote set-url ubuntu@ip-172-31-83-137:~$ git remote add origin https://github.com/arun-choicely/Choicely-aws.git
error: remote origin already exists.
ubuntu@ip-172-31-83-137:~$ 
git remote set-url origin https://github.com/arun-choicely/Choicely-aws.git
git remote -v
git push -u origin master
git push -u origin master --force
ls
cd brand-influencer-platform
ls
cd src
ls
vi App.js
cd..
cd ..
node start
cd src
node start
cd ..
npm start
cd brand-influencer-platform/
ls
cd ..
ls
cd backend
ls
npm start
